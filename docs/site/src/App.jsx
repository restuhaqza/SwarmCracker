        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <a href="https://github.com/restuhaqza/SwarmCracker" target="_blank" rel="noopener noreferrer" className="bg-gradient text-white px-4 py-2 rounded-lg font-semibold hover:opacity-90 transition-opacity">GitHub</a>
            </div>
          </div>
        </div>
      </nav>
      ...
      <section className="pt-32 pb-20 px-4 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          ...
          <div className="flex gap-4 justify-center flex-wrap mb-12">
            <a href="#installation" className="bg-gradient text-white px-8 py-4 rounded-xl font-semibold shadow-lg shadow-primary/30 hover:shadow-primary/40 hover:-translate-y-0.5 transition-all">Get Started</a>
            <a href="https://github.com/restuhaqza/SwarmCracker" target="_blank" rel="noopener noreferrer" className="bg-bg-card border border-border px-8 py-4 rounded-xl font-semibold hover:border-primary transition-colors">View on GitHub</a>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {steps.map((step) => (
              <div key={step.number} className="text-center">
                <div className="w-16 h-16 bg-gradient rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4">{step.number}</div>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            {['manager', 'worker', 'manual'].map((tab) => (
              <button
                ...
                className={`px-6 py-3 rounded-lg font-medium transition-all ${
                  activeTab === tab ? 'bg-gradient text-white' : 'bg-bg-dark text-text-secondary hover:text-white'
                }`}
              >
          ...
          <div className="bg-bg-dark border border-border rounded-2xl overflow-hidden">
            <div className="bg-bg-card px-4 py-3 border-b border-border flex justify-between items-center">
              ...
              <button onClick={() => copyToClipboard(installCommands[activeTab])} className="flex items-center gap-2 text-text-secondary hover:text-primary transition-colors text-sm">
            ...
            <pre className="p-6 overflow-x-auto text-left font-mono text-sm code-syntax"><code>{installCommands[activeTab]}</code></pre>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="bg-gradient rounded-3xl p-12 text-center">
            ...
            <p className="text-lg mb-8 opacity-95">Join the growing community of developers using SwarmCracker for secure, isolated container orchestration.</p>
            <a href="https://github.com/restuhaqza/SwarmCracker" target="_blank" rel="noopener noreferrer" className="inline-block bg-white text-primary px-8 py-4 rounded-xl font-semibold hover:bg-bg-dark hover:text-white transition-colors">View on GitHub</a>
          </div>
        </div>
      </section>
      ...
      <footer className="py-12 px-4 bg-bg-card border-t border-border">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-4 gap-8 mb-8">
            <div>
              <h4 className="font-semibold mb-4 flex items-center gap-2"><span>🔥</span>SwarmCracker</h4>
              <p className="text-text-secondary text-sm">Firecracker MicroVMs with SwarmKit Orchestration</p>
            </div>
            <div>
